...
if ROBOT == "5bar":
    ...
    #save as .dat files
    np.savetxt(os.path.join(dir,'th1des5bar.dat'), np.column_stack((t_vec, th1_vec)), delimiter='\t', header='Time\tth1_vec')
    np.savetxt(os.path.join(dir,'th2des5bar.dat'), np.column_stack((t_vec, th2_vec)), delimiter='\t', header='Time\tth2_vec')
    np.savetxt(os.path.join(dir,'xydes5bar.dat'), np.column_stack((x_vec, y_vec)), delimiter='\t', header='X\tY')
    ...
    th2_vec = th1_th2_vec[:, 1]
    #save as .dat files
    np.savetxt(os.path.join(dir,'th1des2bar.dat'), np.column_stack((t_vec, th1_vec)), delimiter='\t', header='Time\tth1_vec')
    np.savetxt(os.path.join(dir,'th2des2bar.dat'), np.column_stack((t_vec, th2_vec)), delimiter='\t', header='Time\tth2_vec')
    np.savetxt(os.path.join(dir,'xydes2bar.dat'), np.column_stack((x_vec, y_vec)), delimiter='\t', header='X\tY')
    ...
    #Save as .dat files
    # save t_vec, th1_vec as .dat file
    np.savetxt(os.path.join(dir,'th1desdata.dat'), np.column_stack((t_vec, th1_vec)), delimiter='\t', header='Time\tth1_vec')
    # save t_vec, th2_vec as .dat file
    np.savetxt(os.path.join(dir,'th2desdata.dat'), np.column_stack((t_vec, th2_vec)), delimiter='\t', header='Time\tth2_vec')
    # save x_vec, y_vec as .dat file
    np.savetxt(os.path.join(dir,'xydesdata.dat'), np.column_stack((x_vec, y_vec)), delimiter='\t', header='X\tY')
...
def mj_sim(m,d,ttraj,th1traj,th2traj,xy_vec):
    t_fb = np.array(ttraj)
    th1_fb = np.array(th1traj)
    th2_fb = np.array(th2traj)
    xvec=[]
    yvec=[]
    ti=4 #time to reach initial pose
    ...
    with mujoco.viewer.launch_passive(m, d,key_callback=keyboard_func) as viewer:
        while viewer.is_running() and d.time <= t_fb[-1]+ti:
            ...
            print(d.time)
            mujoco.mj_step(m, d)
            plt.plot(d.time,th1,'r.',label='th1')
            plt.plot(d.time,d.qpos[0],'b.',label='th2')
...
try:
    if ROBOT == "5bar":
        ...
        pickle.dump(trajdata, open('traj5bar.pkl', 'wb'))
        #Save data as .dat files
        # save t_fb, th1_fb as .dat file    
        np.savetxt(os.path.join(dir,'th1act5bar.dat'), np.column_stack((t_fb, th1_fb)), delimiter='\t', header='Time\tth1_fb')
        # save t_fb, th2_fb as .dat file
        np.savetxt(os.path.join(dir,'th2act5bar.dat'), np.column_stack((t_fb, th2_fb)), delimiter='\t', header='Time\tth2_fb')


    elif ROBOT == "2bar":
        # Robot operation
        BAUDRATE = 1000000
        DEVICENAME = "/dev/ttyUSB0" #"COM3" #"/dev/ttyUSB0" #COM9 in Windows, /dev/ttyUSB0 in Linux
        ROBOT = "2R"
        BASE_MOTOR = 1
        ELBOW_MOTOR = 2
        ...
        op = False
        while op == False:
            op = my2R.setRobotState([[th1_vec[0],spd],[th2_vec[0],spd]])
            if op == False:
                print("Error in sending data, trying again...")
        ...
        while (abs(e1) + abs(e2)) > tol:
            ...
            e2 = th2_vec[0] - ja2
            # print(f"joint angle errors: {e1:.3f}, {e2:.3f}")
            my2R.setRobotState([[th1_vec[0], spd], [th2_vec[1], spd]])
            sleep(0.2)
        # Pen down
        # my2R.penDown(90)
        ...
        for i in range(len(th1_vec)-1):
            ...
            # Set the robot state for each motor at each time step
            # my2R.setRobotState([[th1_vec[i], th1_dot_vec[i]], [th2_vec[i], th2_dot_vec[i]]])
            fb = my2R.setGetRobotState([[th1_vec[i], 1.5*th1_dot_vec[i]], [th2_vec[i], 1.5*th2_dot_vec[i]]])
            if fb != -1 and abs(th1_vec[i] - fb[0][0]) < 0.15 and abs(th2_vec[i] - fb[1][0]) < 0.15: # Check if feedback is valid and not an outlier
                [[th1_fb[idx], th1_dot_fb[idx]], [th2_fb[idx], th2_dot_fb[idx]]] = fb
                t_fb[idx] = time.time() - start_time
                idx+=1
        ...
        #save data
        trajdata=[x_vec,y_vec,t_vec,th1_vec,th2_vec,th1_dot_vec,th2_dot_vec,t_fb,th1_fb,th2_fb,th1_dot_fb,th2_dot_fb]
        pickle.dump(trajdata, open('traj2bar.pkl', 'wb'))
        #Save data as .dat files
        # save t_fb, th1_fb as .dat file    
        np.savetxt(os.path.join(dir,'th1act2bar.dat'), np.column_stack((t_fb, th1_fb)), delimiter='\t', header='Time\tth1_fb')
        # save t_fb, th2_fb as .dat file
        np.savetxt(os.path.join(dir,'th2act2bar.dat'), np.column_stack((t_fb, th2_fb)), delimiter='\t', header='Time\tth2_fb')
